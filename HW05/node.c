...
void node_delete(node_t* node)
{
	if (node != node_NULL)
	{
		...
		//node->left_child = node_NULL;
		//node->right_child = node_NULL;
		free(node);
	}
}
...
void node_set_child_F(node_t* node, node_t* child, node_child_t position)
{
	...
	if (position == LEFT)
	{
		node_set_left_child_F(node, child);
	}
	else if (position == RIGHT)
	{
		node_set_right_child_F(node, child);
	}
	else if (position == ANY)
	{
		int rnd = rand() % 2;
		node_child_t first_position = (rnd == 0 ? LEFT : RIGHT);
		node_child_t second_position = (first_position == LEFT ? RIGHT : LEFT);
		node_set_child_F(node, child, first_position);
		if (reti == FALSE)
		{
			node_set_child_F(node, child, second_position);
		}
	}
}
...
bool node_set_left_child(node_t* node, node_t* left_child)
{
	...
	if (node_check_null(node_get_left_child(node)) == TRUE)
	{
		...
		node_set_depth(node_get_left_child(node), node_get_depth(node_get_parent(node_get_left_child(node))) + 1);
	}
	return reti;
}
...
bool node_set_right_child(node_t* node, node_t* right_child)
{
	...
	if (node_check_null(node_get_right_child(node)) == TRUE)
	{
		...
		node_set_depth(node_get_right_child(node), node_get_depth(node_get_parent(node_get_right_child(node))) + 1);
	}
	return reti;
}
...
{
	if (node1 == node_NULL || node2 == node_NULL)
		return FALSE;
...
{
	node_child_t reti = NONE;
	if (node != node_NULL)
	{
		node_t* parent = node_get_parent(node);
		if (parent != node_NULL)
		{
			if (node_check_same(node_get_left_child(parent), node) == TRUE)
			{
				reti = LEFT;
			}
			else if (node_check_same(node_get_right_child(parent), node) == TRUE)
			{
				reti = RIGHT;
			}
		}
